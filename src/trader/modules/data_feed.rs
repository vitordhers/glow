...
use tokio::time::{interval, sleep, sleep_until, Instant, Interval};
...
impl DataFeed {
    pub fn new(
        ...
        let tick_data_schema = Schema::from_iter(schema_fields.clone().into_iter());

        let mut indicators_offset = vec![];

        for indicator in indicators {
            let indicator_offset = indicator.get_data_offset();
            indicators_offset.push(indicator_offset);
        ...
        // units profit_and_loss returns balance position action
        ...
        let trading_data_schema = Schema::from_iter(schema_fields.into_iter());
    ...
    loop {
        select! {
            ...
            _ = sleep_until(query_deadline) => {
                ...
                let new_ticks_data =
                    consolidate_complete_tick_data_into_lf(symbols, &ticks_data, trading_data_schema)?.collect()?;
