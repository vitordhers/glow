...
#[derive(Clone)]
pub struct FastSlowRelativeMovingAverages {
    pub name: String,
...
impl Indicator for FastSlowRelativeMovingAverages {
    fn name(&self) -> String {
        self.name.clone()
    }
        ...
        lf = lf
            .with_column(
                when(col(trend_diff_col).is_null())
                    .then(lit(NULL))
                    .otherwise(col(trend_diff_col).rolling_mean(rolling_opts.clone()))
                    .round(momentum_decimals)
                    .alias(trend_lag0),
            )
            .with_column(
