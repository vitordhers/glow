    ...
    fn set_signal_column(&self, lf: &LazyFrame) -> Result<LazyFrame, Error> {
        ...
        signal_lf = signal_lf.with_column(
            ...
            .when(
                ...
                .and(col(close_slow_diff).lt_eq(lit(0.005)).and(col(close_slow_diff).shift(1).gt(lit(0))))
                .and(col(close_fast_diff).lt_eq(lit(0.005)).and(col(close_fast_diff).shift(1).gt(lit(0))))
            )
            .then(lit(1))
            .when(
                col(cloud_top_col).neq(col(close_col)).and(col(cloud_top_col).shift(1).eq(col(close_col).shift(1)))
            ...
            .when(
                ...
                .and(col(close_slow_diff).lt_eq(lit(0.005)).and(col(close_slow_diff).shift(1).gt(lit(0))))
                .and(col(close_fast_diff).gt_eq(lit(-0.01)).and(col(close_fast_diff).shift(1).lt(lit(0))))
            )
            .then(lit(1))
            .when(
                col(cloud_top_col).neq(col(close_col)).and(col(cloud_top_col).shift(1).eq(col(close_col).shift(1)))
...
impl Signal for MultipleStochasticWithThresholdLongSignal {
    ...
    fn set_signal_column(&self, lf: &LazyFrame) -> Result<LazyFrame, Error> {
        ...
        signal_lf = signal_lf.with_column(
            when(
                col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                .and(col(trends_col).shift(1).eq(lit("1234")))
                .and(col(close_slow_diff).shift(1).gt_eq(lit(0.01)))
                .and(col(close_fast_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_fast_diff).shift(1).lt(lit(0.0))))
            )
            .then(lit(1))
            .when(
                col(cloud_top_col).eq(col(close_col))
                .and(col(trends_col).eq(lit("1243")))
                .and(col(close_slow_diff).gt_eq(lit(-0.005)).and(col(close_slow_diff).shift(1).lt(lit(0.0))))
                .and(col(close_fast_diff).gt_eq(lit(-0.01)).and(col(close_fast_diff).shift(1).lt(lit(-0.005))))
            )
            .then(lit(1))
            .when(
                col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                .and(col(trends_col).shift(1).eq(lit("1423")))
                .and(col(close_slow_diff).shift(1).gt_eq(lit(-0.01)).and(col(close_slow_diff).shift(1).lt(lit(0.0))))
                .and(col(close_fast_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_fast_diff).shift(1).lt(lit(0.0))))
            )
            .then(lit(1))
            .when(
                ...
                .and(col(close_slow_diff).lt_eq(lit(0.005)).and(col(close_slow_diff).shift(1).gt(lit(0.0))))
                .and(col(close_fast_diff).lt_eq(lit(0.005)).and(col(close_fast_diff).shift(1).gt(lit(0.0))))
            )
            .then(lit(1))
            .when(
                col(cloud_top_col).eq(col(close_col))
    ...
    fn set_signal_column(&self, lf: &LazyFrame) -> Result<LazyFrame, Error> {
        ...
        signal_lf = signal_lf.with_column(
            ...
            .when(
                ...
                .and(col(close_slow_diff).shift(1).lt_eq(lit(0.01)).and(col(close_slow_diff).shift(1).gt_eq(lit(0.005))))
                .and(col(close_fast_diff).shift(1).gt_eq(lit(0.0)).and(col(close_fast_diff).shift(1).lt_eq(lit(0.005))))
            )
            .then(lit(1))
            // .when(
            //     col(cloud_top_col).neq(col(close_col)).and(col(cloud_top_col).shift(1).eq(col(close_col).shift(1)))
            ...
            .when(
                ...
                .and(col(close_slow_diff).gt_eq(lit(0.0)).and(col(close_slow_diff).shift(1).lt_eq(lit(0.005))))
                .and(col(close_fast_diff).gt_eq(lit(-0.01)).and(col(close_fast_diff).shift(1).lt_eq(lit(-0.005))))
            )
            .then(lit(1))
            .otherwise(lit(0))
            .alias(signal_col),
    ...
    fn set_signal_column(&self, lf: &LazyFrame) -> Result<LazyFrame, Error> {
        ...
        signal_lf = signal_lf.with_column(
            ...
            when(
                    col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                    .and(col(trends_col).shift(1).eq(lit("1234")))
                    .and(col(close_slow_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_slow_diff).shift(1).lt(lit(0.0))))
                    .and(col(close_fast_diff).shift(1).gt_eq(lit(0.01)))
                )
                .then(lit(1))
                .when(
                    col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                    .and(col(trends_col).shift(1).eq(lit("2134")))
                    .and(col(close_slow_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_slow_diff).shift(1).lt_eq(lit(0.005))))
                    .and(col(close_fast_diff).shift(1).gt_eq(lit(0.01)))
                )
                .then(lit(1))
                .when(
                ...
                .when(
                    ...
                    .and(col(close_slow_diff).shift(1).gt_eq(lit(-0.01)).and(col(close_slow_diff).shift(1).lt_eq(lit(-0.005))))
                    .and(col(close_fast_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_fast_diff).shift(1).lt_eq(lit(0.0))))
                )
                .then(lit(1))
                .when(
                    col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                    ...
                    .and(col(close_slow_diff).shift(1).lt(lit(-0.02)))
                    .and(col(close_fast_diff).shift(1).gt_eq(lit(-0.01)).and(col(close_fast_diff).shift(1).lt_eq(lit(-0.005))))
                )
                .then(lit(1))
                .when(
                    col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                ...
                .when(
                    col(cloud_base_col).neq(col(close_col)).and(col(cloud_base_col).shift(1).eq(col(close_col).shift(1)))
                    .and(col(trends_col).shift(1).eq(lit("4132")))
                    .and(col(close_slow_diff).shift(1).gt_eq(lit(-0.01)).and(col(close_slow_diff).shift(1).lt_eq(lit(0.0))))
                    .and(col(close_fast_diff).shift(1).gt_eq(lit(-0.005)).and(col(close_fast_diff).shift(1).lt_eq(lit(0.0))))
                )
                .then(lit(1))
                .otherwise(lit(0))
